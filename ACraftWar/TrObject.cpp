...
bool TrObject::Init(VT(string) path_list, int frame_num, SDL_Point pos, SDL_Renderer* screen)
{
    if (frame_num <= 0)
        return false;
    if (path_list.size() == 0)
...
void TrObject::AddBullet(SDL_Renderer* screen)
{
    ...
    {
        VT(string) path_list = { "image//bul_laser2.png" };
    ...
    {
        VT(string) path_list = { "image//bul_tr.png" };
        pBul = new BulletObj(BulletObj::BL_CIRCLE, BulletObj::BL_DIR_DOWN, 0, 10);
        SDL_Point pos = InitPos();
        pBul->Init(path_list, pos.x, pos.y, screen);
        m_BulletList.push_back(pBul);
